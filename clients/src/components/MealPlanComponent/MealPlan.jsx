...
function MealPlan() {
  ...
  const handlesubmitClick = async (e) => {
    ...
    try {
      ...
      // IMPORTANT: Check your terminal for the exact URL when you run 'npm run serve'
      // It usually looks like this, but replace "YOUR_PROJECT_ID" with your actual Firebase project ID
      const response = await fetch("http://127.0.0.1:5001/full-stack-project-fitness/us-central1/app/api/mealplans/getPlan", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        console.log("SUCCESS! Backend calculated BMR:", data.calculatedCalories);
        console.log("Matched Bucket:", data.bucketMatched);
        console.log("Here is your Meal Plan:", data.plan);
        
        alert(`Plan Found! You need ${data.calculatedCalories} calories. Check console for details.`);
      } else {
      ...
      alert("Failed to connect to the server. Make sure 'npm run serve' is running.");
    }
  }

  return (
    // FIX 1: Changed h-screen to min-h-screen and removed overflow-hidden
    <div className="min-h-screen w-full relative flex flex-col font-sans">
      
      {/* // FIX 2: Changed absolute to fixed, and added -z-10 so the background stays put while scrolling */}
      ...
      {/* MAIN FORM CONTAINER - Added pb-20 for extra padding at the bottom of the scroll */}
      <div className="relative z-10 flex-grow w-full flex justify-center items-start p-10 pb-20">
        <div className="w-full max-w-7xl bg-white/10 backdrop-blur-sm rounded-3xl p-10 flex gap-12 shadow-2xl">
          ...
          <div className="w-1/2 flex flex-col gap-10">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="w-full">
                  <label htmlFor="userGoalWeight" className="text-white font-medium mb-2 block">Goal Weight (kg)</label>
                  <LoginInput
                    id="userGoalWeight"
                    placeHolder={"kg"}
                    icons
            ...
            <div>
              <h3 className="text-2xl font-semibold text-orange-500 mb-6">Goals & Preferences</h3>
              <div className="flex flex-col gap-6">
                <div className="w-full">
                  <label htmlFor="primaryGoal" className="text-white font-medium mb-2 block">Primary Goal</label>
                  <select
                    id="primaryGoal"
                    value={primaryGoal}
                ...
                <div className="flex gap-6">
                  <div className="w-1/2">
                    <label htmlFor="dietaryPreference" className="text-white font-medium mb-2 block">Dietary Preference</label>
                    <select
                  ...
                  <div className="w-1/2">
                    <label htmlFor="cookingAbilities" className="text-white font-medium mb-2 block">Cooking Abilities</label>
                    <select
                      id="cookingAbilities"
          ...
          <div className="w-1/2 flex flex-col gap-10">
            <div>
              <h3 className="text-2xl font-semibold text-orange-500 mb-6">Health Data</h3>
              <div className="flex flex-col gap-6">
                <div className="w-full">
                  <label htmlFor="userAllergies" className="text-white font-medium mb-2 block">Allergies (leave blank if none)</label>
                  <LoginInput
                    id="userAllergies"
                ...
                <div className="w-full">
                  <label htmlFor="userHydration" className="text-white font-medium mb-2 block">Hydration (Glasses of water/day)</label>
                  <LoginInput
                    id="userHydration"
                ...
                <div className="w-full">
                  <label htmlFor="medicalConsiderations" className="text-white font-medium mb-2 block">Medical Considerations</label>
                  <LoginInput
